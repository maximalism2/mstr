export const FETCH_PRICE_BY_ID = 'FETCH_PRICE_BY_ID';
export const FETCHING_LOADING = 'FETCHING_LOADING';

export const FBI_ERROR = 'FBI_ERROR';

export const EDIT_MODE_ON = 'EDIT_MODE_ON';
export const EDIT_MODE_OFF = 'EDIT_MODE_OFF';


export const WILL_REMOVE = 'WILL_REMOVE';
export const REMOVING_LOADING = 'REMOVING_LOADING';
export const REMOVE = 'REMOVE';
export const REMOVE_ERROR = 'REMOVE_ERROR';
export const REMOVING_SUCCESS = 'REMOVING_SUCCESS';

export const RESET_PRICE_VIEW = 'RESET_PRICE_VIEW';

export const MAKE_INPUT = 'MAKE_INPUT';
export const REMOVE_INPUT = 'REMOVE_INPUT';
export const CHANGE_PRODUCT_FIELD = 'CHANGE_PRODUCT_FIELD';
export const CHANGE_MAIN_FIELD = 'CHANGE_MAIN_FIELD';
export const INPUT_INSERT_ERROR = 'INPUT_INSERT_ERROR';

export const REMOVE_PRODUCT = 'REMOVE_PRODUCT';
export const CANCEL_REMOVING_PRODUCT = 'CANCEL_REMOVING_PRODUCT';

export const UPDATING_LOADING = 'UPDATING_LOADING';
export const PRICE_UPDATING_ERROR = 'PRICE_UPDATING_ERROR';
export const PRICE_UPDATING_SUCCESS = 'PRICE_UPDATING_SUCCESS';